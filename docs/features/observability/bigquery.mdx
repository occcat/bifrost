...
Unlike the [OTel](/features/observability/otel) and [Datadog](/features/observability/datadog) connectors, which stream spans to a tracing backend, the BigQuery plugin writes a flat, query-optimized table. It is ideal when you want to own the data, join it against your warehouse, or report on cost and usage with plain SQL.
...
- **[Datadog](/features/observability/datadog)** — Native Datadog APM and LLM Observability integration.
