...
- `prompt_cache.auto_inject` applies to **Claude models only**. Vertex serves Claude and
  Gemini side by side, and only Claude takes a per-block cache marker; Gemini caches
  through a `cachedContent` resource instead, so injection is a no-op there. See
  [Auto-inject cache breakpoints](/providers/supported-providers/anthropic#auto-inject-cache-breakpoints).
