...
Bifrost can also add the marker for clients that send none. Set
`prompt_cache.auto_inject` on the provider and the first cacheable block is marked
automatically, which Bedrock receives as a `cachePoint`. See
